"""Model factory, training loop, and validation helpers."""
...
from spacecraft_anomaly.config import Config, ModelConfig
from spacecraft_anomaly.data.opssat import OpsSatLoader
from spacecraft_anomaly.data.preprocessing import point_adjust
...
def build_model(model_type: str, n_channels: int, cfg: ModelConfig, seq_len: int):
    if model_type == "LSTM_AE":
        from spacecraft_anomaly.models.lstm_ae import LSTMAutoencoder
    ...
    if model_type == "LSTM_VAE":
        from spacecraft_anomaly.models.lstm_ae import LSTMVariationalAE
    ...
    if model_type == "TRANSFORMER":
        from spacecraft_anomaly.models.transformer_ad import TransformerAD
    ...
    if model_type == "GNN":
        from spacecraft_anomaly.models.graph_rnn import GNNVariationalAD
    ...
    if model_type == "FUSION":
        from spacecraft_anomaly.models.fusion import MultiModalAD
        return MultiModalAD(
            n_channels=n_channels,
...
def train(
    ...
    save_dir = str(resolve_path(save_dir))
    print(f"\n{'='*60}")
    print(" Spacecraft Anomaly Detection — Training")
    ...
    if dataset in ("SMAP", "MSL"):
        ...
        loader = OpsSatLoader(channel_id=channel, window_size=window_size)
        loader.summary()
        train_dl, val_dl = loader.get_loaders(batch_size=batch_size)
        n_channels = loader.n_channels
    ...
    for epoch in range(1, epochs + 1):
        ...
        print(
            ...
            f"f1={f1:.4f}  lr={scheduler.get_last_lr()[0]:.2e}  ({elapsed:.1f}s)"
        )
